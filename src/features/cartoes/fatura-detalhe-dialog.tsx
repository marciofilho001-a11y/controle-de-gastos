...
import {
  Clock, Plus, Trash2, Search, Inbox, Sparkles, CheckCheck, CreditCard, CalendarClock, Layers, X,
} from "lucide-react"
...
import { fmtR, fmtMesRef, fmtMesCurto, mesRefAtual, fmtData } from "@/lib/format"
import {
  faturaDoMes, mesesDoCartao, sugestoesParcelasParaMes,
} from "@/lib/selectors"
import { cn } from "@/lib/utils"

export function FaturaDetalheDialog({
  cartao,
  open,
  onOpenChange,
  mesRefBase,
  ...
  const meses = useMemo(() => {
    if (!cartao) return []
    return mesesDoCartao(cartao.id, transacoes, compras, faturaItens, mesRefBase)
  }, [cartao, transacoes, compras, faturaItens, mesRefBase])

  // mês selecionado padrão: último com fatura, senão o base
  const mesAtivo = useMemo(() => {
    if (mesSel && meses.includes(mesSel)) return mesSel
    if (!cartao) return mesRefBase
  ...
  async function addTodasSugestoes() {
    if (!cartao || !dados?.sugestoes.length) return
    setBusy(true)
    try {
      ...
      <DialogContent className="max-w-5xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2.5">
            <span className="grid size-9 place-items-center overflow-hidden rounded-lg bg-muted text-muted-foreground ring-1 ring-border">
        ...
        <div className="grid gap-4 md:grid-cols-[190px_1fr]">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    sel ? "border-primary/40 bg-primary/10" : "border-transparent hover:bg-secondary"
                  )}
                >
                  <span className={cn("text-sm font-medium capitalize", sel && "text-primary")}>{fmtMesRef(m)}</span>
          ...
          <div className="flex max-h-[62vh] flex-col gap-3 overflow-y-auto">
            <div className="flex items-baseline gap-2">
              <h3 className="font-display text-lg font-semibold capitalize">{fmtMesRef(mesAtivo)}</h3>
              ...
              <div className="rounded-xl border border-primary/25 bg-primary/5 p-3">
                ...
                <div className="flex flex-col gap-1.5">
                  {dados.sugestoes.map((s, i) => {
                    const Icon = catInfo(s.categoria).icon
                    return (
                      <button
                        key={i}
                      ...
                      >
                        <span className="grid size-8 place-items-center rounded-lg" style={{ background: `${catColor(s.categoria)}1f`, color: catColor(s.categoria) }}>
            ...
            <div className="flex flex-col gap-1.5">
              {dados.itens.length === 0 ? (
                <div className="grid place-items-center gap-2 rounded-lg border border-dashed py-6 text-center text-sm text-muted-foreground">
              ...
              {!dados.planejando && !catFiltro && dados.restante > 0.005 && (
                <div className="flex items-center justify-between rounded-lg border-l-[3px] border-muted-foreground/40 bg-card px-3 py-2 text-sm">
...
function ItemRow({
  ...
  const Icon = catInfo(item.categoria).icon
  const cor = catColor(item.categoria)
  return (
    <div className="flex items-center gap-2.5 rounded-lg border-l-2 bg-card px-3 py-2" style={{ borderLeftColor: cor }}>
      <span className="grid size-8 shrink-0 place-items-center rounded-lg" style={{ background: `${cor}1f`, color: cor }}>
        <Icon className="size-4" />
      </span>
